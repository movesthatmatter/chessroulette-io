...
import { chessHistoryToSimplePgn } from '../../chessGame/util';
import { chessGameTimeLimitMsMap } from '../../metadata/game';
import { seconds } from '../../util';
import { addMoveToChessHistory, getHistoryBranch } from '../analysisActions';
import { pgnToChessHistory } from '../testUtil';

test('empty history', () => {
  const history: ChessRecursiveHistory = [];

  const actual = getHistoryBranch(history);
  const expected: ChessRecursiveHistory = [];
...
test('history without branches w/o given index', () => {
  ...
  const actual = getHistoryBranch(history);
  const expected: ChessRecursiveHistory = history;
...
test('history without branches w given index as NestdedBranchIndex and move Index out of boundaries', () => {
  const pgn = '1. e4 c5 2. Nf3';
...
test('history with 1 level branch w given branched index', () => {
  const pgn = '1. e4 c5 2. Nf3';
