...
export const isChessRecursiveHistoryIndex = (a: unknown): a is ChessRecursiveHistoryIndex =>
  Array.isArray(a) &&
...
export const getBranchAtIndex = (
  history: ChessRecursiveHistory,
  index: ChessHistoryIndex
): undefined | ChessRecursiveHistory => {
  ...
  if (!isChessRecursiveHistoryIndex(nestedIndex)) {
    return branch;
  }
...
export const incrementChessRecursiveHistoryIndex = (
  index: ChessRecursiveHistoryIndex
): ChessRecursiveHistoryIndex => {
  const [moveIndex, branchIndex, nestedBranchIndex] = index;

  if (isChessRecursiveHistoryIndex(nestedBranchIndex)) {
    return [moveIndex, branchIndex, incrementChessRecursiveHistoryIndex(nestedBranchIndex)];
  }
